...
## UI refresh rollout — extending the "register language" design system beyond Machines/Rentals/Maintenance

A prior pass applied a new, denser "register language" design system (tokens in `globals.css`, three new `packages/ui` parts — `AllocationBar`, `AttentionStrip`, `AvailabilityLane`) to exactly the two screens its approved canvas depicted (Machines, Rentals, Maintenance). This pass extended it to the rest of the app, but deliberately not everywhere — the two new components replace a specific pattern (a hand-rolled tab-strip + separate KPI-card-row, and a buried-in-description-text derived count), and were applied only where that pattern genuinely existed:

- **Did get `AllocationBar` + `AttentionStrip`**: Requirements' Open Market (`needs_response/responded/in_auction/closed` tab-strip → segments; added a validity-closing-soon attention item), Quotations' list (7-status tab-strip → segments; the separate "Requested" pseudo-filter stayed its own section, surfaced instead as an attention item so it isn't misrepresented as a quotation status), Billing (`draft/issued/overdue/paid/cancelled` tab-strip → segments; added a client-recomputed overdue/due-soon pair that catches invoices still "issued" past their due date, ahead of the server-only status transition — same reasoning as the existing per-row `DueSoonBadge`), Transport (`Select` status filter → segments; added a "planned past its scheduled date" attention item, real given `plannedDate` already exists on the contract).
- **Got only the `AttentionStrip` half**: Logsheets — `customerConfirmed` is a boolean, not a real status distribution, so no `AllocationBar` (that would be filter-bar weight for two segments nobody asked to filter by); fixed a pre-existing inconsistency instead (`customerConfirmed` was rendered via an ad-hoc `<Badge>` instead of `StatusBadge` + a map, unlike every other status in the app) and added an attention item for logsheets unconfirmed >7 days.
- **Got neither, confirmed not assumed**: Auctions (one auction at a time per requirement, no multi-row list to summarize — only fix was making `auction.status` go through `StatusBadge`+a map like everything else instead of an ad-hoc `<Badge>`), Catalogue (no status/segment concept on static reference data — only fix was `font-mono` on count cells for the numeric-cell convention), Dashboard (`AttentionPanel` is already a richer, already-shipped equivalent of `AttentionStrip` with linked rows and per-item tone — swapping it for the plainer strip would have been a regression, not a refresh; only fix was routing its error state through `ErrorState` instead of a raw `<p>`), Settings/Projects/Work-orders (pure admin config or too low-cardinality to want at-a-glance segment counts).

No backend/contract changes were needed anywhere in this pass — every segment count and attention signal is computable from fields the API already returns.
